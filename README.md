...
## 目前內容規模
- 靜態題庫 505 題：國語、自然常識、地理歷史，各分 3–4 歲／5–6 歲／7–9 歲／10–12 歲四級。
- 數學與音樂是程式即時出題，每次都不同，等於出不完。
- 出題採抽牌式（deckDraw）：每個「科目＋年齡」有一副洗好的牌，抽過的不放回，整副抽完才重洗，牌堆跟著小朋友的存檔走。同一輪內也不會出現相同的問句。數學與音樂用 genUnique 記住最近 70 題的指紋來擋重複。
- 一局要答幾題可在首頁選 5 / 10 / 15 / 20 題；每答對 5 題變形完成一台機甲，題數多就換下一台進機庫。
- 音樂科有會實際發聲的聽力題（比高低、數拍子、聽唱名、聽音程、上下行），用 Web Audio 產生，不需要音檔。
- 三層獎勵，總共 34 件收藏品：5 台機甲（用累積星星換，門檻 0/5/15/32/55）、20 種塗裝（每台 4 種，靠該台出擊 0/6/15/30 次解鎖）、9 件裝備（武器護具背包，條件分散在各科目）。
- 每台機甲在「收藏庫」裡都可以自由變形來回，也可以換塗裝、裝上武器。
